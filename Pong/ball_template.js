...
export default class Ball {
    ...
    update(deltaT) {
        ...
        const centerIncrement = new THREE.Vector3(coveredDistance * Math.cos(this.direction),coveredDistance * Math.sin(this.direction), 0.0);
        ...
        if (centerIncrement.x < 0) { // The ball is moving to the left
            if (this.center.x - this.radius <= this.player1.center.x + this.player1.halfSize.x && 
                ...
                this.center.y + this.radius >= this.player1.center.y - this.player1.halfSize.y
                    ...
                    this.direction = -this.direction + Math.PI; // The ball rebounds
            }
        }
        ...
        if (this.center.y - this.radius <= (-this.table.halfSize.y) || // The ball is moving down and hit the bottom line
             this.center.y + this.radius >= (this.table.halfSize.y)) { // The ball is moving up and hit the top line
